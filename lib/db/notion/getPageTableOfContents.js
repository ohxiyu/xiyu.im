...
export const getPageTableOfContents = (page, recordMap) => {
  ...
  for (const tocItem of toc) {
    ...
    // 兜底：如果 indentLevel 不是有效数字（type 不在 indentLevels 表里），统一当 0
    const actual = Number.isInteger(indentLevel) ? indentLevel : 0

    // 最多迭代次数防御：栈深 + 一次插入足够，避免任何意外 destructure undefined / 死循环
    let safety = 8
    do {
      const prevIndent = indentLevelStack[indentLevelStack.length - 1]
      // guard：当连续 pop 把栈清空时，以当前 actual 为新根，结束循环
      if (!prevIndent) {
        const lvl = Math.max(0, actual)
        indentLevelStack.push({ actual, effective: lvl })
        tocItem.indentLevel = lvl
        break
      }

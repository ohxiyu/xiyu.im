...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  console.log(`[${taskId}] 🕒 开始解析文章: ${logSlug} @ ${new Date().toISOString()}`)
  ...
  if (
    ...
    try {
      ...
      console.warn(`[${taskId}] [resolvePostProps] cache-bust refresh failed:`, e)
    }
    const step3bEnd = Date.now()
    console.log(`[${taskId}] ⏱ 强刷缓存重查耗时: ${step3bEnd - step3bStart}ms @ ${new Date().toISOString()}`)
  }
  ...
  if (!post) {
    ...
    console.warn(
      `[${taskId}] ❌ POST_NOT_FOUND fullSlug="${fullSlug}" lastSegment="${lastSegment}" ` +
  ...
  if (post) {
    ...
    try {
      ...
      console.warn(`[${taskId}] [resolvePostProps] processPostData failed`, e?.message || e)
    }
    ...
    console.log(`[${taskId}] ✓ post 已就绪 slug=${sanitizeForLog(post?.slug)} hasBlockMap=${!!props.post?.blockMap} paywall=${!!props.post?.paywall}`)
  ...
  console.log(`[${taskId}] ✅ 完成解析文章: ${logSlug}, 总耗时: ${endTime - startTime}ms @ ${new Date().toISOString()}`)

const CONFIG = {
  ...
  // —— 关于页 ——
  // 头部「在做」那一行（和首页「在想」同款）。首页那行是从最近文章 tags 自动聚合的，
  // 关于页这行是手写的——它讲的是长期在做什么，不该随最近写了什么而变。
  XIYU_ABOUT_DOING: ['Agent', '自托管', '长期主义'],
  ...
  XIYU_ABOUT_TIMELINE: [
    ...
    {
      when: '2026 —',
      // ⚠️ 别改回「用 AI Agent 给自己造系统」——那和头部大字（BIO）一字不差，
      // 隔一屏重复一次
      title: 'Agent 与自托管',
      fallback:
  ...
  // 拆问题的三个框架。接在时间轴最后一个时期后面，是一段话不是三张卡——
  // Notion 原文里它们本来就是一句话。
  XIYU_ABOUT_METHODS_LINE:
    '拆问题基本靠三样东西：秩分析找出最少的独立变量，其余都是它们的线性组合；第一性原理拆到不可再拆的事实，然后从零重建；人性透镜看激励从哪来——谁在为什么样的结果买单。',

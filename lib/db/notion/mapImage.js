...
/**
 * Notion 已把主域从 notion.so 迁到 notion.com。
 * 下面这些判断原先只认 notion.so，一旦图片 URL 来自新域名就会走错分支——
 * 该跳过的被重复转换、该转换的被当成已转换，表现为图片裂开或加载不出。
 * 统一改成「两个域名都认」。
 */
const NOTION_IMAGE_PATH_RE = /^https:\/\/www\.notion\.(so|com)\/image/
const NOTION_PAGE_COVER_RE = /notion\.(so|com)\/images\/page-cover/
const NOTION_IMAGES_RE = /^https:\/\/www\.notion\.(so|com)\/images\//
...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (!isEmoji(ret) && !NOTION_PAGE_COVER_RE.test(ret)) {

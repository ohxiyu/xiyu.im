 ...
 * - 勾选框可能出现的 on/off
 */
function parseConfigEnabled(enableValue) {
  const value = normalizeConfigValue(enableValue)
  if (typeof value === 'boolean') return value
  if (typeof value === 'number') return value === 1
  if (typeof value !== 'string') return false

  return ['yes', 'true', '1', 'on', 'enable', 'enabled', '是', '启用'].includes(

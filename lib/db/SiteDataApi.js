...
import { applyPaywall } from '../paywall/post'
 ...
 * 这份站点数据是否「拿到了真东西」
 *
 * 用来区分「Notion 确实没有已发布文章」和「这次没拉到」。后者结构完整但列表为空，
 * 既不该写进缓存，也不该被当成稳定结果长时间静态化。
 */
export function hasUsablePages(data) {
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  return getOrSetDataWithCacheIf(cacheKey,
    // 只有真的拿到文章才写缓存。Notion 偶发超时/限流时 convertNotionToSiteData
    // 会返回 EmptyData（结构完整但没有文章），它是真值、照样会被缓存下来，
    // 结果一次瞬时抖动就把首页固化成空列表——尤其在 revalidate 拉长之后。
    hasUsablePages,
    async () => {
...
/**
 * 净化要写进日志的外部输入
 * 去掉 % 前缀的格式化占位符、换行、控制字符，并限长，
 * 避免用户可控的 slug 造成格式化字符串注入或伪造日志行
 */
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // slug 可能来自 HTTP 请求（/api/unlock），直接拼进日志会造成格式化字符串注入
  // （console.* 支持 %s/%d）和换行伪造日志，这里统一净化后再用于日志
  const logSlug = sanitizeForLog(fullSlug || lastSegment)
  const taskId = `${logSlug}-${Date.now()}` // 当前任务唯一标识
  // 统一日志出口：格式化字符串恒为字符串字面量，任何动态/用户可控内容
  // 只作为参数传入。console.* 的第一个参数才会被当作格式串解析（%s/%d），
  // 这样即可根除 js/tainted-format-string
  const logInfo = (msg, ...rest) => console.log('[%s] %s', taskId, msg, ...rest)
  const logWarn = (msg, ...rest) => console.warn('[%s] %s', taskId, msg, ...rest)

  const startTime = Date.now()
  logInfo(`🕒 开始解析文章: ${logSlug} @ ${new Date().toISOString()}`)

  // 拉全站数据
  const step1Start = Date.now()
  const props = await fetchGlobalAllData({ from: source, locale })
  const step1End = Date.now()
  logInfo(`⏱ fetchGlobalAllData 耗时: ${step1End - step1Start}ms @ ${new Date().toISOString()}`)

  // 工具函数：查找文章
  const findPost = () => {
    if (!props?.allPages) return null
    const isPostLike = p => p && !p.type?.includes('Menu')
    const decodedFullSlug = (() => {
      try { return decodeURIComponent(fullSlug) } catch { return fullSlug }
    })()
    ...
    return (
      // 1. 完整 slug 完全匹配
      props.allPages.find(p => isPostLike(p) && p.slug === fullSlug) ||
      // 1b. 解码后再匹配（处理中文/特殊字符 URL）
      props.allPages.find(p => isPostLike(p) && p.slug === decodedFullSlug) ||
      // 2. UUID 匹配
      props.allPages.find(p => p?.id === fullSlug) ||
      props.allPages.find(p => p?.id === decodedFullSlug) ||
      // 3. 容错：URL 最后一段命中存储 slug 的尾段（前缀 mismatch 时兜底）
      props.allPages.find(p => isPostLike(p) && p.slug && p.slug.split('/').pop() === lastSegment) ||
      props.allPages.find(p => isPostLike(p) && p.slug && p.slug.split('/').pop() === decodedLastSegment) ||
      // 4. 容错：lastSegment 直接是裸 slug（无前缀的老文章）
      props.allPages.find(p => isPostLike(p) && p.slug === lastSegment) ||
      props.allPages.find(p => isPostLike(p) && p.slug === decodedLastSegment) ||
      null
    )
  }
  ...
  // logInfo(`⏱ 查找文章耗时: ${step2End - step2Start}ms @ ${new Date().toISOString()}`)
  ...
  if (!post && typeof lastSegment === 'string' && /^[a-f0-9-]{32,36}$/i.test(lastSegment)) {
    ...
    try {
      ...
      logWarn(`[resolvePostProps] fetchPageFromNotion failed:`, lastSegment, e)
    }
    const step3End = Date.now()
    logInfo(`⏱ fetchPageFromNotion 耗时: ${step3End - step3Start}ms @ ${new Date().toISOString()}`)
  }

  // 3b. 仍然找不到 → 主页 ISR 已经看到新文章但本路由的 fetchGlobalAllData 缓存还是旧的
  //     强制清掉站点数据缓存重拉一次再找。仅对非 UUID 路径触发，避免每次 404 都额外打 Notion。
  //     冷却保护：进程级全局 5 分钟一次，防止爬虫扫描无效 slug 打爆 Notion API 配额。
  if (
    !post &&
    typeof lastSegment === 'string' &&
    !/^[a-f0-9-]{32,36}$/i.test(lastSegment) &&
    Date.now() - (globalThis.__xiyu_lastCacheBust || 0) > 5 * 60 * 1000
  ) {
    globalThis.__xiyu_lastCacheBust = Date.now()
    const step3bStart = Date.now()
    try {
      const siteIds = (BLOG.NOTION_PAGE_ID || '').split(',') || []
      for (const siteId of siteIds) {
        const id = extractLangId(siteId)
        if (id) await delCacheData(`site_${id}`)
      }
      const fresh = await fetchGlobalAllData({ from: `${source}-refresh`, locale })
      props.allPages = fresh.allPages
      post = findPost()
    } catch (e) {
      logWarn(`[resolvePostProps] cache-bust refresh failed:`, e)
    }
    const step3bEnd = Date.now()
    logInfo(`⏱ 强刷缓存重查耗时: ${step3bEnd - step3bStart}ms @ ${new Date().toISOString()}`)
  }

  // 诊断日志：还是没找到时把 allPages 状态吐到 Vercel 日志，方便排查
  if (!post) {
    const all = props?.allPages || []
    const totalPages = all.length
    const postsCount = all.filter(p => p?.type === 'Post').length
    const slugSnippet = all
      .map(p => p?.slug)
      .filter(Boolean)
      .filter(s => typeof s === 'string')
      .filter(s => {
        const ls = (lastSegment || '').toLowerCase()
        return !ls || s.toLowerCase().includes(ls.slice(0, 8))
      })
      .slice(0, 10)
    ...
    logWarn(
      `❌ POST_NOT_FOUND fullSlug="${logSlug}" lastSegment="${sanitizeForLog(lastSegment)}" ` +
      `allPages=${totalPages} (Posts=${postsCount}) ` +
      `similar_slugs=${JSON.stringify(slugSnippet)} ` +
      `first5_sample=${JSON.stringify(sample)}`
    )
  }
  ...
  const ensureBlockMap = async (post) => {
    ...
    try {
      ...
      logWarn(`[resolvePostProps] fetchNotionPageBlocks failed:`, post.id, e)
    }
    const step4End = Date.now()
    logInfo(`⏱ ensureBlockMap 耗时: ${step4End - step4Start}ms @ ${new Date().toISOString()}`)
    return post
  }
  ...
  if (post) {
    ...
    try {
      ...
      logWarn(`[resolvePostProps] processPostData failed`, e?.message || e)
    }
    // hard guarantee：哪怕 processPostData 内部哪一步把 props.post 删了，这里再赋一次
    if (!props.post) props.post = post
    // 付费文章：剥离正文，只留预览。必须在返回给 getStaticProps 之前执行，
    // 否则 blockMap 会被序列化进 __NEXT_DATA__，付不付费都能看到全文。
    applyPaywall(props, {
      previewBlocks: Number(process.env.PAYWALL_PREVIEW_BLOCKS || 2)
    })
    logInfo(`✓ post 已就绪 slug=${sanitizeForLog(post?.slug)} hasBlockMap=${!!props.post?.blockMap} paywall=${!!props.post?.paywall}`)
    // logInfo(`⏱ processPostData 耗时: ${step5End - step5Start}ms @ ${new Date().toISOString()}`)
  } else {
    props.post = null
  }
  ...
  logInfo(`✅ 完成解析文章: ${logSlug}, 总耗时: ${endTime - startTime}ms @ ${new Date().toISOString()}`)
...
async function convertNotionToSiteData(
  ...
  // schema 为空是个「静默杀手」：getPageProperties 靠 schema[key] 才能把 Notion 的
  // 内部属性 ID 翻译成 slug/status/type，schema 空 → 所有属性都读不出来 →
  // 文章没有 slug/status → 被 allPages 的过滤条件整批丢弃 → 首页列表空，
  // 但页面本身完全正常，日志里一句报错都没有。这里显式点名。
  if (Object.keys(schema).length === 0) {
    console.error(
      `[${traceId}] ❌ COLLECTION_SCHEMA_EMPTY collectionId="${collectionId}" ` +
        `collectionKeys=${JSON.stringify(Object.keys(pageRecordMap.collection || {}).slice(0, 5))} ` +
        `— 取不到 collection 或其 schema，所有文章的属性都会解析失败，首页会变成空列表。` +
        `常见原因：数据库结构被改动/迁移导致 collection_id 变化，或 Notion 返回了新版数据结构。`
    )
  }
  ...
  // 首页空列表的根因定位：区分「没抓到页面」和「抓到了但属性没解析出来」
  if (allPages.length === 0) {
    const sample = collectionData.slice(0, 3).map(p => ({
      slug: p?.slug ?? null,
      status: p?.status ?? null,
      type: p?.type ?? null
    }))
    console.error(
      `[${traceId}] ❌ ALLPAGES_EMPTY pageIds=${pageIds.length} ` +
        `collectionData=${collectionData.length} schemaKeys=${Object.keys(schema).length} ` +
        `sample=${JSON.stringify(sample)} — ` +
        (pageIds.length === 0
          ? '一个页面 ID 都没抓到，通常是 collection_query/视图数据没返回。'
          : Object.keys(schema).length === 0
            ? 'schema 为空，属性无法解析（见上方 COLLECTION_SCHEMA_EMPTY）。'
            : '抓到了页面但都没有 slug 或 status 不是 Published/Invisible。')
    )
  }
